...
# initialize the voideo stream and allow the cemera sensor to warmup
with open('yolov5_config.json','r',encoding='utf8')as fg:
    opt = json.load(fg)
    print('[INFO] YOLOv5 Cofig:',opt)

darknet = Darknet(opt)
if darknet.webcam:
    # cudnn.benchmark = True  # set True to speed up constant image size inference
    dataset = LoadStreams(darknet.source,img_size=opt["imgz"],  stride=darknet.stride)
else:
    ...
    for path, img, img0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):
            if darknet.webcam:  # batch_size >= 1
                feed_type_curr, p, s, im0, frame = "Camera_%s" % str(i), path[i], '%g: ' %i,
            else:
                feed_type_curr, p, s, im0, frame = "Camera", path, '', img0s, getattr(dataset)
            ...
            if det is not None and len(det):
                ...
                for c in det[:, -1].unique():
                    n = (det[:, -1] == c).sum() # detections per calss
...
def video_feed(feed_type):
    ...
    if feed_type == 'Camera_0':
        return Response(detect_gen(dataset=dataset, feed_type=feed_type),
                        mimetype="multipart/x-mixed-repalce; boundary=frame")
